...
- **Read-only AI tools**: The AI can query data (recipes, history, and ratings when available) but never writes directly
...
- **Recipe formatting**: Paste freeform text → AI reformats into restricted Markdown (Title, Intro, Ingredients, Preparation, Cooking) → user reviews preview → approves to save
- **Meal plan generation**: AI queries recipe list, history, and ratings (ratings deferred) → proposes a weekly plan of day/recipe assignments → user reviews and edits → approves to apply
